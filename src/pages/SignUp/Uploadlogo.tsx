...
export default function Uploadlogo(props:any) {
  ...
  const schema= Joi.array().required();
   ...
   const postData = async (user: any)=> {
      ...
      user["specialty_ids[]"].map((id: string) => data.append('specialty_ids[]', id));
  ...
  const handleChange = (event: any) =>{
    setValues((values) => [...values,(event.target as HTMLInputElement).files[0]]) 
    console.log(values)
        };
