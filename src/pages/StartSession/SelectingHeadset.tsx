...
export default function SelectingHeadset(props: any) {
    ...
    const handleSubmit = async (event: any) => {
        ...
        if (error) {
            ...
            .then(result => { setSessionId(result.data.id)
                             console.log(result);})
    ...
    const selectHeadset =async () => {
        const token = await (window as any).electronAPI.getPassword("token");
        fetch(`${config.apiURL}/api/v1/doctors/center_headsets?center_id=${props.centerId}`,{
            method: 'Get',
            redirect: 'follow',
            headers: {'Authorization': `Bearer ${token}`}
        })
            ...
            <Menu>
               ...
               <MenuList>
               {
                   ...
                   <MenuItem 
                       ...
                       >
                           {headset.attributes.name}</MenuItem>
                   )))
               }
               </MenuList>
             </Menu>
             <Text fontSize="10px" color="red">{errors.selectedHeadset}</Text>
        </ModalBody>
        <ModalFooter>
           <Button
               w="214px"
